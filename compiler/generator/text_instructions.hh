...
class TextInstVisitor : public InstVisitor {
   ...
   public:
    ...
    /*
        gLessParenthesis avoid some parenthesis in BinopInst instructions by following C operator precedence.
        But since parenthesis are also added by CastInst, SelectInst, FuncallInst instructions,
        any generated expression may still have some more parenthesis than strictly necessary.
     */
    virtual bool needParenthesis(BinopInst* inst, ValueInst* arg)
    {
        // Simple values like numbers or variables do not need parenthesis
        if (arg->isSimpleValue() || dynamic_cast<FunCallInst*>(arg)) {
            return false;
